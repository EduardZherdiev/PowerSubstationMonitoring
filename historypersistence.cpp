...
QVector<EventRecord> loadEvents()
{
    ...
    for (const QJsonObject &object : loadObjects(eventsFilePath())) {
        ...
        if (!timestamp.isValid() || timestamp < cutoff
            || levelValue < static_cast<int>(EventLevel::Info)
            || levelValue > static_cast<int>(EventLevel::Critical)) {
            continue;
        }

        records.append(EventRecord{timestamp,
                                   static_cast<EventLevel>(levelValue),
                                   object.value(QStringLiteral("source")).toString(),
                                   object.value(QStringLiteral("message")).toString()});
        retainedObjects.append(object);
    }
    std::sort(records.begin(), records.end(), [](const EventRecord &left, const EventRecord &right) {
