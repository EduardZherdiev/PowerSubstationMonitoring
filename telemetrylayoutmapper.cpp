...
{
    return QCoreApplication::translate("TelemetryLayoutMapper", "%1 kV")
        .arg(QString::number(value, 'f', 1));
}

QString formatA(double value)
{
    return QCoreApplication::translate("TelemetryLayoutMapper", "%1 A")
        .arg(QString::number(value, 'f', 1));
}

QString formatC(double value)
{
    return QCoreApplication::translate("TelemetryLayoutMapper", "%1 C")
        .arg(QString::number(value, 'f', 1));
}
...
void apply(SubstationLayout::Layout *layout, const SensorSnapshot &snapshot)
{
    ...
    for (SubstationLayout::NodeSpec &node : layout->nodes) {
        ...
        if (node.id == QStringLiteral("Line-1")) {
            ...
            node.status = snapshot.breakerClosed ? QStringLiteral("Closed")
                                                 : QStringLiteral("Open");
        } else if (node.id == QStringLiteral("Bus-1")) {
            node.status = snapshot.breakerClosed ? QStringLiteral("Energized")
                                                 : QStringLiteral("De-energized");
            node.parameters[QStringLiteral("Voltage")] = snapshot.breakerClosed
                                                             ? formatKv(snapshot.sourceVoltageKv)
                                                             : formatKv(0.0);
        } else if (node.id == QStringLiteral("TR-1")) {
            node.parameters[QStringLiteral("Temperature")] = formatC(
            ...
            node.parameters[QStringLiteral("Voltage")] = snapshot.breakerClosed ? formatKv(12.0)
                                                                                : formatKv(0.0);
            node.parameters[QStringLiteral("Current")] = snapshot.breakerClosed
                                                             ? formatA(snapshot.sourceCurrentA)
                                                             : formatA(0.0);
            node.status = snapshot.breakerClosed ? QStringLiteral("Loading")
            ...
            node.status = snapshot.breakerClosed ? QStringLiteral("Ready")
                                                 : QStringLiteral("De-energized");
            node.parameters[QStringLiteral("Voltage")] = snapshot.breakerClosed ? formatKv(12.0)
                                                                                : formatKv(0.0);
            node.parameters[QStringLiteral("Current")] = snapshot.breakerClosed
