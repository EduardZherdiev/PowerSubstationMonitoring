...
bool saveToFile(const QString& path, const Layout& layout, QString* errorMessage) {
    ...
    for (const NodeSpec& node : layout.nodes) {
        ...
        nodeObject.insert(QStringLiteral("position"), QJsonObject{{QStringLiteral("x"), node.position.x()},
                                                                  {QStringLiteral("y"), node.position.y()}});
        nodeObject.insert(QStringLiteral("size"), QJsonObject{{QStringLiteral("width"), node.size.width()},
    ...
    if (!file.commit()) {
        if (errorMessage) {
            *errorMessage =
                QCoreApplication::translate("SubstationLayout", "Unable to finalize layout file: %1").arg(path);
        }
        return false;
    }
    return true;
